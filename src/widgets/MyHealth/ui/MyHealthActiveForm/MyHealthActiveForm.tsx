...
export const MyHealthActiveForm = memo(
  ({ activeCategory, userProfile }: Props) => {
    ...
    const componentByStep: Record<MyHealthCategory, ReactNode> = {
      ...
      [MyHealthCategory.Food]: (
        <MyHealthFoodForm userProfile={userProfile} onSuccess={onSuccess} />
      ),
    ...
    return (
      <div className={styles.container}>{componentByStep[activeCategory]}</div>
    );
  }
);
