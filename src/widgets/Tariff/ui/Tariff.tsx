...
import { PricingCard, PricingCardProps } from '../../PricingPlans/ui/PricingCard';
...
export const Tariff = (user: any) => {
  ...
  const [selectedPlan, setSelectedPlan] = useState<ExtendedPricingCardProps | null>(null);
  ...
  return (
    <div className={styles.main}>
          ...
          buttons={[
            {
              ...
              children: (
                <Typography className={styles.toggleButtonText}>
                  <span style={showYears ? { color: '#000' } : { color: '#fff' }}>Ежегодно - </span>
                  <span style={showYears ? { color: '#158c3d' } : { color: '#01FC54' }}>
                    Скидка 20%{' '}
                  </span>
                </Typography>
              ),
            ...
            {
              value: false,
              children: <Typography className={styles.toggleButtonText}>Ежемесячно</Typography>,
            },
          ]}
        />
      </Stack>
        ...
        {pricingCards.map((card, idx) => {
          ...
          if (idx === pricingCards.length - 1) {
            ...
            billingText = showYears
              ? `Счёт выставляется ежегодно на ${new Intl.NumberFormat('ru-RU', {
          ...
          let buttonText =
            card.buttonText === 'Связаться' ? card.buttonText : `Выбрать ${card.title}`;
          ...
          if (
            ...
            if (activeTariff.tariff.tariff_type === (showYears ? 'year' : 'month')) {
              ...
              buttonText = `Продлить ${activeTariff.tariff.tariff}`;
              buttonColor = 'lightgray';
            }
          }
      ...
      {selectedPlan && (
        <PaymentWidget
          plan={selectedPlan}
          finalPrice={showYears ? selectedPlan.rawPrice * 12 * 0.8 : selectedPlan.rawPrice}
          user={user.user}
        />
      )}
    </div>
