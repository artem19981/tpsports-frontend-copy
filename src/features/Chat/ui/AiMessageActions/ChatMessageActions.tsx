...
import { ChatDto, ChatVariant } from 'features/Chat/model';
...
export const ChatMessageActions = memo(
  ({ message_id, text, rate, chatVariant }: Props) => {
    ...
    const likeMessage = () => {
      rateMessage({
        ...
        queryClient.setQueryData(['chat', chatVariant], (prevChat: any) => {
          const newMessages = prevChat.messages.map(
            (message: ChatDto['messages'][0]) => {
